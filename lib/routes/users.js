...
module.exports = function(server, options) {
  ...
  return [
    ...
    },
    {
      ...
      path: '/users/list',
      config: {
        description: 'Get beers, noting which are drank, for a user',
        tags: ['api'],
        validate: {
          headers: Joi.object({
            authorization: Joi.string()
      ...
      handler: function(request, reply) {
        ...
        var CheckedBeers = request.model.checked_beers;
        var Users = request.model.users;
        var Lists = request.model.lists;
        var user = request.auth.credentials.user;
        ...
        if (user){
          Users.findOne().where({'id': user.id}).populate('beers').then(function(user){
            Lists.findOne(user.list).populate('beers').then(function(list){
              return reply({drank: user.beers, list: list.beers});
            })
            .catch(function(error){
              return reply(Boom.wrap(error));
            });
